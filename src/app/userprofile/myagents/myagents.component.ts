...
export class MyagentsComponent implements OnInit {
  ...
  constructor(private nearlukservice: NearlukService, private router: Router,private toastr: ToastrService, private title: Title, private meta: Meta) {
    this.title.setTitle('NearLuk | Get Verified Tenanats & Owners');
    this.meta.addTags([
      { name: 'description', content: 'Agents get a customized web platform and mobile app to manage vast properties across different locations.' },
      { name: 'keywords', content: 'Agents get a customized web platform and mobile app to manage vast properties across different locations.' }
    ]);
