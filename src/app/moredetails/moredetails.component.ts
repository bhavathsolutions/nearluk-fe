...
export class MoredetailsComponent implements OnInit {
  ...
  constructor(private acr: ActivatedRoute, private nearlukservice: NearlukService, private router: Router, private toastr: ToastrService, private title: Title, private meta: Meta) {
    this.title.setTitle('More Detail | flats to rent near me | rental property | 1bhk flat on rent | 2bhk flat on rent | property for lease');
    this.meta.addTags([
      { name: 'description', content: 'More detail will provide the complete information about the property including with Facilities & Amenities and owner contact details.' },
      { name: 'keywords', content: 'commercial property for rent, Nearluk, commercial space for rent, retail space for rent, commercial lease ' }
    ]);
    ...
    if (event.index == 1) {
      ...
      if (sessionStorage.getItem('user') != null) {
        this.nearlukservice.getSectionid(sessionStorage.getItem('user')).subscribe((data) => {
          ...
          this.nearlukservice.interestCheck(this.propertyId, this.userid).subscribe((data) => {
            // console.log(data.data[0].property_interested)
            if (data.data[0].property_interested === true) {
              this.isShown = false;
  ...
  interest() {
    if (sessionStorage.getItem('user') != null) {
      ...
      this.nearlukservice.getSectionid(sessionStorage.getItem('user')).subscribe((data) => {
        this.userid = data[0].userid;
        this.roleid = data[0].roleid;
        this.nearlukservice.postinterset(this.propertyId, this.userid).subscribe((data) => {


          this.toastr.success('Added to interest');
          this.nearlukservice.getOwnersInfo(this.propertyId, this.userid).subscribe((data) => {
            // console.log(data)
            this.ownerdata = data.data
            this.isShown = false;
    ...
    this.nearlukservice.getMoreDetails(this.propertyId, this.session_id).subscribe((data) => {
      ...
      if (data.result == false) {
        ...
        if (data.data == "NDF") {
            ...
            for (var i = 0; i < facilityid.length; i++) {
              for (var j = i; j <= i; j++) {
                this.facilitiesArray.push({
                  "fid": facilityid[i],
                  'fname': facilityName[i],
                  "fimg": this.imgApi + "/" + facilityName[i] + '.png'
                })
              }
            }
          }
          ...
          if (amenityid != null) {
            ...
            for (var i = 0; i < amenityid.length; i++) {
              for (var j = i; j <= i; j++) {
                ...
                this.amenitiesArrayGet.push({
                  "aid": amenityid[i],
                  'avalue': amenitiesValue[i],
                  'aname': amenityName[i], "aimg": this.imgApi + "/" + amenityName[i] + '.png'
                })
              }
            }
          }
          ...
          this.videoDisplay = this.imgApi + "/" + this.propertyId + '.mp4';
        }
      }
    });
